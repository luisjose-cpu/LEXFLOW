...
When a browser session has a valid LEXFLOW JWT, the global search layer calls the cloud API. If the API is unavailable, the UI falls back to deterministic demo data so pilot demos remain usable.
...
- Search consumes the cloud API when authenticated. Some client and case detail panels still use deterministic demo data until the web app fully maps every backend resource into editable forms.
