...
El comando ejecuta `cloud:preflight`, luego `cloud:smoke`, consulta `/api/v1/status`, `/readiness` y `/version`, y genera un JSON local en `reports/cloud/lexflow-cloud-release-*.json`. El reporte incluye modos de proveedores externos, `production_ready`, `public_production_ready`, conteo y claves de blockers/warnings de readiness para seguimiento operativo. No incluye passwords, tokens, secretos ni datos de tenants; `reports/` queda fuera de git.
