...
def test_render_blueprint_declares_required_runtime_contracts() -> None:
    ...
    assert "REQUIRE_VERIFIED_DOCUMENT_DOWNLOADS" in render_yaml
...
def test_fast_production_gate_requires_public_security_env_contract() -> None:
    ...
    for key in ["LEXFLOW_WEB_URL=", "CREDENTIAL_ENCRYPTION_KEY=", "MALWARE_SCANNER_PROVIDER=", "REQUIRE_VERIFIED_DOCUMENT_DOWNLOADS=", "REQUIRE_BILLING_WEBHOOK_SIGNATURE=", "RESTORE_DRILL_VERIFIED_AT=", "RESTORE_DRILL_MAX_AGE_HOURS=", "REQUIRE_OWNER_MFA=", "FAILED_LOGIN_BACKEND=", "FAILED_LOGIN_LIMIT=", "FAILED_LOGIN_WINDOW_MINUTES="]:
        assert key in gate
    for key in ["LEXFLOW_WEB_URL", "CREDENTIAL_ENCRYPTION_KEY", "MALWARE_SCANNER_PROVIDER", "REQUIRE_VERIFIED_DOCUMENT_DOWNLOADS", "REQUIRE_BILLING_WEBHOOK_SIGNATURE", "RESTORE_DRILL_VERIFIED_AT", "RESTORE_DRILL_MAX_AGE_HOURS", "REQUIRE_OWNER_MFA", "FAILED_LOGIN_BACKEND", "FAILED_LOGIN_LIMIT", "FAILED_LOGIN_WINDOW_MINUTES"]:
